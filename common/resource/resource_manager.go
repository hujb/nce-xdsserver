...
// GetInstance sync.Once类型，它提供了一种保证函数只会被执行一次的机制。在GetInstance()函数中，我们使用了once.Do()方法，并将创建单例对象的逻辑放在了括号中的函数中。这样，在每次调用GetInstance()时，都会检查once的值，如果为Once{}，则执行括号中的函数创建单例对象，并将其赋值给instance，然后返回instance。如果once的值不为Once{}，则直接返回instance。
